...
def build_trainer(name,
    ...
    class trainer_cls(base):
        ...
        def __init__(self, config=None, env=None, logger_creator=None, load_env=None, env_configs=None):
        ...
        def _init(self, config, env_creator):
            ...
            if make_policy_optimizer:
                envs = {env_name: self.load_env(env_config) for env_name, env_config in self.env_configs.items()}
                self.optimizer = make_policy_optimizer(self.workers, envs, config)
            else:
...
from .custom_optimizer import make_policy_optimizer
