...
def generate_scenario_tree(look_ahead_period,n_sample_scenario,interest_samples,cash_return, outflow_samples, json_write_file):
	...
	for ts in range(1,look_ahead_period):
		...
		for p in parent_nodes:
			for s in range(n_sample_scenario[ts-1]):
				for cout in range(len(outflow_samples)):
					cur_child = {"time_step": ts, "zc_price": cash_return, "liability": outflow_samples[cout]}
