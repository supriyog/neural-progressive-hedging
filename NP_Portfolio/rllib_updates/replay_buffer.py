...
@DeveloperAPI
class ConsecutiveReplayBuffer(object):
    @DeveloperAPI
    def __init__(self, size, bias):
        ...
        self._est_size_bytes = 0
        self._bias = bias
        print("Consecutive Replay Buffer Loaded!!!!!")
    ...
    def sample_idxes(self, batch_size):
        ...
        if bias > 0:
            ...
            ran = np.random.geometric(bias) - 1
            while ran > n - batch_size:
                ran = np.random.geometric(bias) - 1

            idx = cur - batch_size - ran 
            if idx < 0:
                idx += n

            while ((idx < cur and idx > cur - batch_size) or
                (idx < cur + n and idx > cur - batch_size + n)):
            ...
            ran = np.random.randint(n - batch_size + 1)
        ...
        if idx <= n - batch_size:
            ...
            idxes = list(range(idx,n)) + list(range(0,batch_size-n+idx))
        return idxes
