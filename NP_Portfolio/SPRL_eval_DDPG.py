...
parser.add_argument("--rf", type=str, default="result_DDPG_v1")
...
#json_write_file = cur_work_dir+"Dataset/scenario_" + str(result_file) + ".json"
...
for exp_num in range(experiment_number, experiment_number+1):
	...
	while not done:
		...
		action_to_ddpg = action# np.array([0.044, 0.125, 0.51, 0.06, 0.034, 0.077, 0.15])
...
np.save(cur_work_dir+'results/{}/Returns_{}_{}_{}.npy'.format(experiment_file,result_file,stock_selction_number,experiment_number), all_rewards)
np.save(cur_work_dir + 'results/{}/Weights_{}_{}_{}.npy'.format(experiment_file,result_file,stock_selction_number,experiment_number), all_weights)
