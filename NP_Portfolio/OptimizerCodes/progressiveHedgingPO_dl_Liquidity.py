...
class stocProgPH:
	...
	def __init__(self, tree_file_name, eval_num_assets, config_file_name, current_allocation, result_file_name, alpha_cv):
		...
		self.scen_tree_nodes, self.node_data = utilities_liquidity.readJasonScenarioFile(tree_file_name)
		...
		for sub_scen in range(self.num_subscenarios):
			...
			self.sibling_scenarios[sub_scen] = utilities_liquidity.getSiblingScenarios(sub_scen_nodes, self.horizon_length, self.node_level, self.scen_tree_nodes)
		...
		for scen in range(self.num_subscenarios):
			...
			self.slave_prob[scen].parameters.threads.set(1)
			self.slave_prob[scen].parameters.parallel.set(1)
	...
	def solveStocProgMaster(self, weights_from_rl):
		...
		while stop_flag :
			...
			utilities_liquidity.write_results("total residual value, dual objective, learning rate, time, np-alpha at iteration " +str(self.global_count) +" ### " + str(total_residual_value) + " ### "+str(dual_average_total_value) + " ### "+str(self.learning_rate_lambda)+" ### "+str(time.time()-initial_time)+" ### "+str(self.alpha_neural_proximal_rate), self.result_file)
			...
			if(self.global_count >= self.max_iterations):
				utilities_liquidity.write_results("total residual value at convergence is " +str(self.global_count) +" ### " + str(total_residual_value) +" ### " + str(self.learning_rate_lambda), self.result_file)
				stop_flag = 0
		...
		utilities_liquidity.write_results("Total time taken is: "+str(time.time()-initial_time),self.result_file)
	...
	def solveStocProgSlaves(self, parallel_option, scen_number, lambda_values, lambda_y, average_Xvalues, average_Yvalue, warm_start_sol, send_end):
		...
		if self.global_count == 0:
			...
			for scen_count in range(len(self.scen_id_in_subscenarios[scen_number])):
				for time_step in range(self.horizon_length):
					my_sense1 = "E"			# Sense of constraint, E, L, or G.
					row1[0] = 0.0#sum(self.liabilitiy_amounts[scen_id[scen_count]][time_step]) # r.h.s. constant of the constraint #time_step*self.num_assets + asset_num
					if (time_step == 0):
						row1[0] = (-1.0) * self.initial_cash_value + sum(self.liabilitiy_amounts[scen_id[scen_count]][time_step+1])
