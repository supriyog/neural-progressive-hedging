...
class DataGenerator(object):
    ...
    def __init__(self, history, steps=730, window_length=50, start_idx=0, steps_test = 0): #, start_idx=0, start_date=None):
        """
    ...
    def reset(self):
        ...
        if self.steps == 0:
            ...
            self.idx = np.random.randint(
                low=self.window_length + self.start_idx, high=self._data.shape[2] - self.steps)
        ...
        stock_universe_select = np.random.randint(low=0, high=self._data.shape[0])
        data = self._data[stock_universe_select,:, self.idx - self.window_length:self.idx + self.steps + 1, :5]
        # apply augmentation?
        self.data = data
        print ("Data reset shape: "+str(self.data.shape))
        cash_outflow_mu = 0.025
        cash_outflow_sigma = 0.01
        cash_outflows_samples = np.random.normal(cash_outflow_mu, cash_outflow_sigma, 30)
        cash_outflow_requirments = np.cumsum(cash_outflows_samples)
        return self.data[:, self.step:self.step + self.window_length, :].copy(), \
               self.data[:, self.step + self.window_length:self.step + self.window_length + 1, :].copy(), cash_outflow_requirments
