...
class DataGenerator(object):
    ...
    def __init__(self, history, dates, stocks, steps=730, window_length=50, start_idx=0, synthetic_data_config=None): #, start_idx=0, start_date=None):
        """
        ...
        self.stocks = stocks
        self.dates = dates
        self.synthetic_data_config = synthetic_data_config or dict()
    ...
    def reset(self):
        ...
        # apply augmentation?
        start_date = self.dates[self.idx - self.window_length].strftime('%Y-%m-%d')
        end_date = self.dates[self.idx + self.steps].strftime('%Y-%m-%d')
        GBM.GBM_Generator(self.stocks, start_date=start_date, end_date=end_date, **self.synthetic_data_config)
