...
def train(rllib_env_configs, rllib_trainer_config, rllib_trainer_cls,
          policies_dirpath, max_steps=1e6, env_cls=None):
    register_env("portfolio_env", lambda env: utils.load_env(rllib_env_configs[list(rllib_env_configs.keys())[0]], env_cls))
    for env_name,env_config in rllib_env_configs.items():
        register_env(env_name, lambda env: utils.load_env(env_config, env_cls))
    trainer = rllib_trainer_cls(config=rllib_trainer_config, env="portfolio_env", load_env=utils.load_env, env_configs=rllib_env_configs)
...
def main(args=None):
    ...
    rllib_env_configs = yaml.load(rllib_env_config_filepath.open('r'), Loader=yaml.FullLoader)
    ...
    train(rllib_env_configs, rllib_trainer_config, rllib_trainer_cls,
