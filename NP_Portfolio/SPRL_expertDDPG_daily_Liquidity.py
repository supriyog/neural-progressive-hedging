...
from OptimizerCodes.progressiveHedgingPO_dl_Liquidity import stocProgPH
from utils import utilities_liquidity
...
parser.add_argument("--rf", type=str, default="result_NPC_v01")
...
if(os.path.exists(cur_work_dir + "results/" + experiment_file + "/" + str(result_file) + "_" +str(stock_selction_number) +"_" + str(experiment_number) +"_" + str(alpha_cvar)+ "_" + str(outflow_sigma) + ".txt")):
	shutil.move(cur_work_dir + "results/" + experiment_file + "/" + str(result_file) + "_" +str(stock_selction_number)  +"_" + str(experiment_number) +"_" + str(alpha_cvar) +"_" + str(outflow_sigma)  + ".txt", "results/" + str(result_file) + "_" +str(stock_selction_number) + "_backup.txt")
sp_result_file = cur_work_dir + "results/" + experiment_file + "/" + str(result_file) + "_" +str(stock_selction_number)+ "_"+ str(experiment_number)+"_" + str(alpha_cvar) +"_" + str(outflow_sigma) + ".txt"
...
cash_outflows_train = np.load(cur_work_dir + 'data/prestored_samples/cash_outflow_train_{}_T{}.npy'.format(outflow_sigma,experiment_number+1))
...
for exp_num in range(experiment_number,experiment_number+1):
	...
	while not done:
		...
		outflow_samples = cash_outflows_train[time_step] + np.sum(cash_outflows_test[:time_step]) #np.random.normal(mu, sigma, 10)
		tree_location = utilities_liquidity.generate_scenario_tree(look_ahead_period, n_sample_scenario, interest_samples,cash_return, outflow_samples, json_write_file)
		...
		utilities_liquidity.write_results("SP average alloc results in exp " + str(exp_num) + " iteration "+str(eval_steps)+" is ## " + str(sum(root_node_results)) + " ## " + str(root_node_results), sp_result_file)
		utilities_liquidity.write_results("Current holding value in exp " + str(exp_num) + " iteration "+str(eval_steps)+" ## " + str(sum(current_allocation)) + " ## " + str(current_allocation), sp_result_file)
		utilities_liquidity.write_results("Buy-sell policy is in exp " + str(exp_num) + " iteration "+str(eval_steps)+" ## " + str(sum(final_policy_value)) + " ## " + str(final_policy_value),sp_result_file)
		utilities_liquidity.write_results("SP_Portfolio value in exp " + str(exp_num) + " next period in iteration "+str(eval_steps)+" ## " + str(sum(init_allocation)) + " ## " + str(init_allocation),sp_result_file)
		...
		current_cash_alloc = init_allocation_ddpg_order[0]
		...
		utilities_liquidity.write_results("Portfolio value in next period in exp " + str(exp_num) + " iteration "+str(eval_steps)+" ## " + str(sum(init_allocation_ddpg)) + " ## " + str(init_allocation_ddpg), sp_result_file)
		utilities_liquidity.write_results("Cash allocations; total outflow: cur outflow: total alloc: cur alloc: " + str(exp_num) + " iteration "+str(eval_steps)+" ## " + str(np.sum(cash_outflows_test[:time_step+1])) + " ## " + str(cash_outflows_test[time_step])+" ## " + str(current_cash_alloc) + " ## " + str(prev_cash_alloc), sp_result_file)

		prev_cash_alloc = current_cash_alloc
		...
		if eval_steps >= max_eval_steps:
			...
			np.save(cur_work_dir + 'results/{}/Returns_{}_{}_{}_{}.npy'.format(experiment_file, result_file, alpha_cvar,stock_selction_number, experiment_number), all_rewards)
			np.save(cur_work_dir + 'results/{}/Weights_{}_{}_{}_{}.npy'.format(experiment_file, result_file, alpha_cvar,stock_selction_number,experiment_number), all_weights)
